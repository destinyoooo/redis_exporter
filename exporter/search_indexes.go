...
func (e *Exporter) extractSearchIndexesMetrics(ch chan<- prometheus.Metric, c redis.Conn) {
	...
	for _, index := range searchIndexes {
		...
		e.registerConstMetricGauge(ch, "search_index_avg_per_doc_records", indexInfo.RecordsPerDocAvg, indexInfo.IndexName)
		e.registerConstMetricGauge(ch, "search_index_avg_per_record_bytes", indexInfo.BytesPerRecordAvg, indexInfo.IndexName)
		e.registerConstMetricGauge(ch, "search_index_avg_per_term_offsets", indexInfo.OffsetsPerTermAvg, indexInfo.IndexName)
		e.registerConstMetricGauge(ch, "search_index_avg_per_record_offset_bits", indexInfo.OffsetBitsPerRecordAvg, indexInfo.IndexName)
